...
Write-Host 'PowerShell Script: Main-ReportsPublishPreperation.ps1'
...
if ($ConfigurationName -eq "Publish")
	...
	if ($MailToProdNode -ne $null) { 
		...
		$MailToProdNode.value = 'MSTONE@scufcwfunds.com;DHuynh@scufcwfunds.com'
		# Save the changes back to the app.config
		$configXml.Save($path)
	}
